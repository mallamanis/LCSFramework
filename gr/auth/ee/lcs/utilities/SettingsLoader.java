...
public class SettingsLoader {
	...
	private void loadProperties(String filename) throws IOException{
		final FileInputStream input = new FileInputStream("defaultLcs.properties");
		lcsProperties.load(input);
	}
	
	private double getNumericProperty(String propertyName, double defaultValue) {
		try {
			return Double.parseDouble(lcsProperties.getProperty(propertyName,Double.toString(defaultValue)));
		} catch( Exception ex) {
			return defaultValue;
		}
	}
	
	public static double getNumericSetting(String propertyName, double defaultValue) {
	...
	public static String getStringSetting(String propertyName, String defaultValue) {
