...
/**
 * Selects and adds the best classifier (based on fitness)
 * from the inital ClassifierSet to the target set.
...
public class BestClassifierSelector implements INaturalSelector {
	...
	/**
	 * Boolean indicating if the selector selects the 
	 * best or worst classifier.
	 */
	 ...
	 * Selects the best classifier in the fromPopulation 
	 * 		and adds it to toPopulation
	 * @param howManyToSelect the numerosity that the best classifier
	 * 			 is going to be added
	 * @param fromPopulation the source set of classifiers
	 * @param toPopulation the target set of classifiers. In this 
	 * 			set the best classifier will be added
	 */
	public void select(int howManyToSelect, ClassifierSet fromPopulation,
	  ...
	  if (bestIndex == -1)
		  return;
	  toPopulation.addClassifier(new Macroclassifier(
			  fromPopulation.getClassifier(bestIndex), howManyToSelect), true);
  }
