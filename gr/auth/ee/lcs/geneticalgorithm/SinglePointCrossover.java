...
/**
 *  A binary genetic operator that performs
 *  gene crossover at a single point.
...
public class SinglePointCrossover implements IBinaryGeneticOperator {
	...
	public Classifier operate(Classifier classifierA, Classifier classifierB) {
	   ...
	   */
	  int mutationPoint = (int) Math.round(
			  Math.random() * chromosomeSize - 1);
	  child.chromosome = performCrossover(
			  classifierA.getChromosome(),
			  classifierB.getChromosome(),
			  mutationPoint);
	...
	protected ExtendedBitSet performCrossover(ExtendedBitSet chromosomeA,
	  ...
	  child.setSubSet(position, chromosomeB.getSubSet(
			  position, chromosomeB.size() - position));
