...
public class WeightedRouletteSelector implements INaturalSelector {
	 ...
	 * @param fromPopulation the ClassifierSet from 
	 * 	which the selection will take place
	 * @param toPopulation the ClassifierSet to which
	 *  the selected Classifers will be added
	 */
	public void select(int howManyToSelect,
	  ...
	  for (int i = 0; i < howManyToSelect; i++){
		  ...
		  do{
			  selectedIndex++;
			  tempSum += fromPopulation.getClassifierNumerosity(selectedIndex)
			  		* fromPopulation.getClassifier(selectedIndex).fitness;			  
		  } while (tempSum < rand);	  
		  //Add selectedIndex
		  toPopulation.addClassifier(new Macroclassifier(
				  fromPopulation.getClassifier(selectedIndex), 1), false);
	  } //next roulette
