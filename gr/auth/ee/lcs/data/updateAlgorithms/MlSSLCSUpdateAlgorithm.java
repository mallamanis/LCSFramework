...
public class MlSSLCSUpdateAlgorithm extends UpdateAlgorithmFactoryAndStrategy {
	...
	public MlSSLCSUpdateAlgorithm(final double reward, final double penaltyPercent, final int labels, final IGeneticAlgorithmStrategy geneticAlgorithm,
	...
	/* (non-Javadoc)
	 * @see gr.auth.ee.lcs.data.UpdateAlgorithmFactoryAndStrategy#cover(gr.auth.ee.lcs.classifiers.ClassifierSet, int)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see gr.auth.ee.lcs.data.UpdateAlgorithmFactoryAndStrategy#performUpdate(gr.auth.ee.lcs.classifiers.ClassifierSet, gr.auth.ee.lcs.classifiers.ClassifierSet)
	 */
	@Override
	...
	protected void updateSet(ClassifierSet population, ClassifierSet matchSet,
		...
		for (int lbl = 0; lbl < numberOfLabels; lbl++) {
			...
			for (int i = 0; i < matchSetSize; i++) {
				if (matchSet.getClassifier(i).classifyLabelCorrectly(instanceIndex, lbl) == 1)
