...
/** 
 *  An interface for representing different update strategies,
 *  depending on the LCS algorithm. The implementing objects are 
 *  factories because they instantiate update algorithm-specific
 *  objects for the classifiers. They also represent a strategy 
...
public abstract class  UpdateAlgorithmFactoryAndStrategy {
  ...
  /**
   * The static strategy used throughout the algorithms. 
  ...
  /**
   * Returns the implementation specific attribute that 
   * represents the classifier's comparison to the other's.
   * @param aClassifier the classifier's value to be returned
   * @param mode the mode for the comparison values
