...
public class ASLCSUpdateAlgorithm extends AbstractSLCSUpdateAlgorithm {
  ...
  public void updateFitness(Classifier aClassifier, int numerosity, ClassifierSet correctSet) {
	  ...
	  aClassifier.fitness 
	  		= Math.pow(((double) (data.tp)) / (double) (data.msa), n);
		...
		switch (mode) {
			case COMPARISON_MODE_EXPLORATION:
				return aClassifier.fitness 
						* (aClassifier.experience < 8? 0 : 1);
			case COMPARISON_MODE_DELETION:
				data = (GenericSLCSClassifierData) aClassifier.updateData;
				return aClassifier.fitness 
						* ((aClassifier.experience < 20) ? 
								100. : Math.exp(-(Double.isNaN(data.ns) ? 1 : data.ns) + 1))
						* (((aClassifier.getCoverage() == 0) 
								&& aClassifier.experience == 1) ? 0. : 1);
					 //TODO: Something else?		
			case COMPARISON_MODE_EXPLOITATION:
				data = (GenericSLCSClassifierData) aClassifier.updateData;
				return (((double) (data.tp)) 
						/ (double) (data.msa)); 					
		}
		return 0;
	}
