...
/**
 * A bridge [GoF] that decouples the chromosome bit
 * representation and the data-set specific meaning.
...
public abstract class ClassifierTransformBridge {
   ...
   *  @param visionVector the vision input to be tested
   *  @param chromosome the chromosome testing to vision 
   ...
   *  @param aClassifier the classifier to convert to Sting
   ...
   *  @param visionVector the vision vector to cover
   *  @param advocatingAction the action the random classifier
   *  	will advocate for
  ...
  /** 
   *  Tests the given chromosomes if the baseClassifier
   *  is a more general version of the testClassifier.
   *  @param baseClassifier the base classifier
   *  @param testClassifier the test classifier
   *  @return true if base is more general than test
   */
  public abstract boolean isMoreGeneral(
		  Classifier baseClassifier, Classifier testClassifier);
  ...
  /** 
   *  Fixes a chromosome bit representation 
   *  in the correct value range (e.g. after mutation or crossover)
   ...
   * @param aClassifier the classifier from which to obtain the classification
   * @return the class
   * TODO: Pending changes on ml
   */
  public abstract int getClassification(Classifier aClassifier);
  
  /**
   * Sets the classification as specified by the representation.
   * @param aClassifier the classifier to set the classification
   ...
   * @param toBeFixed the classifier to be fixed
   */
   ...
   * @param classifier the classifier to convert
  ...
  /**
   * Each implementation of the ClassifierTransformBridge might choose
   * to save additional data on each classifier. This data is a 
   * Serializable object and is representation-specific.
   * @param aClassifier the classifier to set the object
   */
