...
public int totalNumerosity = 0;
   ...
   *  It checks if the classifier already exists and increases its numerosity.
  ...
  public void addClassifier(Macroclassifier macro, boolean thoroughAdd) {
	  ...
	  /*
	   * No matching or subsumable more general classifier found.
	   * Add and control size...
	   */
  ...
  public int getClassifierNumerosity(Classifier aClassifier) {
	  for (int i = 0; i < myMacroclassifiers.size(); i++){
		  if (myMacroclassifiers.elementAt(i).myClassifier.getSerial()
  ...
  /** 
   *  removes a micro-classifier from the set.
   *  It either completely deletes it (if the classsifier's numerosity is 0)
   *  or by decreasing the numerosity
   */
  public void deleteClassifier(Classifier aClassifier) {
	  		 
	  int index;
	  for (index = 0; index < myMacroclassifiers.size(); index++){
		  if (myMacroclassifiers.elementAt(index).myClassifier.
				  getSerial() == aClassifier.getSerial()) break;
	  }
  ...
  /**
   * Postprocessing in the classifier set. 
   * Simply remove all classifiers with less than a minimum
