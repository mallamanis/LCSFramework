...
public class Classifier implements Serializable{
  ...
  /**
   * A float showing the number of instances that the rule has covered.
   * Used for calculating coverage.
   */
  transient protected int covered = 0;
  
  /**
   * The number of instances we have checked so far.
   * Used for coverage
   */
  ...
  /** 
   *  The timestamp is the last iteration
   *  the classifier has participated in a GA Evolution.
   */
  ...
  public boolean equals(Classifier anotherClassifier) {
	  return ClassifierTransformBridge.instance.areEqual(
			  this, anotherClassifier);
	  }
  ...
  public boolean isMatch(double[] visionVector) {
	  return ClassifierTransformBridge.instance.isMatch(
			  visionVector, chromosome);
  }
  ...
  public boolean isMatch(int instanceIndex) {
	  ...
	  if (this.matchInstances[instanceIndex] == -1) {
		  this.matchInstances[instanceIndex] =
			  (byte) ((ClassifierTransformBridge.instance.isMatch(
