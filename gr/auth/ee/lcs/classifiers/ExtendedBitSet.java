...
/**
 * <p>Title: ExtendedBitSet</p>
 * <p>Description: Bit string class that implements a more flexible set of operators than</p>
 * <p>java.util.BitSet, though it implements all the routines of that class</p>
 * <p>so as to make this one plug-compatible.  It also emulates BitSet's</p>
 * <p>functionality of transparently extending the set with zero bits if a bit</p>
 * <p>outside its current length is accessed.</p>
...
public class ExtendedBitSet
  ...
  public ExtendedBitSet(String bitPattern) {
    ...
    for (int i = lenBits; i > 0; i -= BITSINLONG) {
      ...
      for (int j = 0; j < BITSINLONG && lenBits - j > 0; ++j) {
        ...
        //Shifts the bits mask to the left, by 1, with 0's shifted in from the right.
        mask <<= 1;
      }
  ...
  public ExtendedBitSet(int length, long randomSeed) {
    ...
    bitUnits[bitUnits.length - 1] &= -1L >>> (BITSINLONG - length); //Zero-fill right shift
  }
  ...
  public ExtendedBitSet(ExtendedBitSet bitSet) {
    ...
    System.arraycopy(bitSet.bitUnits, 0, bitUnits, 0, bitSet.bitUnits.length);
  }
    ...
    if (obj instanceof ExtendedBitSet) {
      ...
      if (this == other) {
        ...
        int limit = (bitUnits.length < other.bitUnits.length) ?
            bitUnits.length : other.bitUnits.length;
  ...
  /**
   * Sets a range of bits to false.  <code>length</code> bits starting
   * at <code>offset</code> are set to false.
   *
   * @param offset start of range
   * @param length length of range
   * @return this
