...
public class SortPopulationControl implements IPopulationControlStrategy {
	...
	public void controlPopulation(ClassifierSet aSet) {
		...
		Macroclassifier[] list = new Macroclassifier[aSet.getNumberOfMacroclassifiers()];
		...
		Comparator<Macroclassifier> comp = new Comparator<Macroclassifier>(){
			...
			@Override
			public final int compare(final Macroclassifier MacroA, final Macroclassifier MacroB) {
				
				
				final double fitnessA = MacroA.myClassifier.getComparisonValue(comparisonMode);
				final double fitnessB = MacroB.myClassifier.getComparisonValue(comparisonMode);
